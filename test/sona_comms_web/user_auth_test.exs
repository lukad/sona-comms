defmodule SonaCommsWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SonaCommsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SonaCommsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
